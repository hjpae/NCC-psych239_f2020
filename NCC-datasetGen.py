...
class NCCDataGen(object):
    ...
    def GMM(self, K, meanSampStd, varSampStd, size):
        ...
        sampler.weights_ = abs(np.random.randn(K, 1)) # unnormalized 
        sampler.weights_ = sampler.weights_ / sum(sampler.weights_) # mixture weights sum to one
    ...
    def Yn(self, dKnot, x):
        ...
        return self.normalize(
            self.normalize(interpolate.PchipInterpolator(supportX, np.random.randn(dKnot))(x.flatten()))[:,
            np.newaxis] + self.noise(x))
    ...
    def noise(self, x):
        ...
        vij = interpolate.UnivariateSpline(supportX, np.random.uniform(0, 5, len(supportX)))(noise.flatten())[:,
              np.newaxis]
    ...
    def Run(self):
        ...
        with open(self.saveFile, "w") as saveFile:
            for idx, size in enumerate(sizeList):
                ...
                ## saving causal
                xncausal = copy.deepcopy(xn)  ## to protect from random shuffle
                causalData = {"trainX": xncausal.ravel().tolist(), "trainY": yn.ravel().tolist(), "label": 0,
                              "type": "causal", "size": int(size), "index": idx}
                anticausalData = {"trainX": yn.ravel().tolist(), "trainY": xncausal.ravel().tolist(), "label": 1,
                                  "type": "anticausal", "size": int(size), "index": idx}

                np.random.shuffle(xn)

                independentData = {"trainX": xn.ravel().tolist(), "trainY": yn.ravel().tolist(), "label": 0.5,
                                   "type": "independent-causal", "size": int(size), "index": idx}
                # saving causal
                json.dump(causalData, saveFile)
                saveFile.write("\n")

                # saving anti causal
                json.dump(anticausalData, saveFile)
                saveFile.write("\n")
...
if __name__ == "__main__":
    obj = NCCDataGen(saveFile = "./causal-data-gen-30K.json-original")
    obj.Run()
