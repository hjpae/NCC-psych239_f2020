...
class NCC(nn.Module):
    def __init__(self, trainSplitRatio=0.7, sizeEmbLayer=100, sizeClassfLayer=100, dropOutRatio=0.75,):
        ...
        self.embedLayer = nn.Sequential(
            nn.Linear(2, self.sizeEmbLayer), #1
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeEmbLayer, self.sizeEmbLayer), #2
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeEmbLayer, self.sizeEmbLayer), #3
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeEmbLayer, self.sizeEmbLayer), #4
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeEmbLayer, self.sizeEmbLayer), #5
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
        ...
        self.classLayer = nn.Sequential(
            nn.Linear(self.sizeEmbLayer, self.sizeClassfLayer), #1
            nn.BatchNorm1d(self.sizeClassfLayer),
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeClassfLayer, self.sizeClassfLayer), #2
            nn.BatchNorm1d(self.sizeClassfLayer),
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeClassfLayer, self.sizeClassfLayer), #3
            nn.BatchNorm1d(self.sizeClassfLayer),
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeClassfLayer, self.sizeClassfLayer), #4
            nn.BatchNorm1d(self.sizeClassfLayer),
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
            nn.Linear(self.sizeClassfLayer, self.sizeClassfLayer), #5
            nn.BatchNorm1d(self.sizeClassfLayer),
            nn.ReLU(),
            nn.Dropout(self.dropOutRatio),
    ...
    def forward(self, xVal, yVal):
        ...
        e1L3 = self.e1Linear2(e1D2).view(BatchSize, self.sizeClassfLayer, DataSize)
        e1B3 = self.e1BatchNorm2(e1L3).view(BatchSize, DataSize, self.sizeClassfLayer)                        
        e1R3 = self.e1ReLu2(e1B3)
        e1D3 = self.e2Drouput2(e1R3)

        e1L4 = self.e1Linear2(e1D3).view(BatchSize, self.sizeClassfLayer, DataSize)
        e1B4 = self.e1BatchNorm2(e1L4).view(BatchSize, DataSize, self.sizeClassfLayer)                        
        e1R4 = self.e1ReLu2(e1B4)
        e1D4 = self.e2Drouput2(e1R4)

        e1L5 = self.e1Linear2(e1D4).view(BatchSize, self.sizeClassfLayer, DataSize)
        e1B5 = self.e1BatchNorm2(e1L5).view(BatchSize, DataSize, self.sizeClassfLayer)                        
        e1R5 = self.e1ReLu2(e1B5)
        e1D5 = self.e2Drouput2(e1R5)

        e1L6 = self.e1Linear2(e1D5).view(BatchSize, self.sizeClassfLayer, DataSize)
        e1B6 = self.e1BatchNorm2(e1L6).view(BatchSize, DataSize, self.sizeClassfLayer)                        
